...
public class UserProfileController {
    ...
    @Autowired
    private com.example.buildpro.service.ComplaintService complaintService;
    ...
    @GetMapping("/orders/product-service/{orderId}")
    public String viewProductReport(@PathVariable Long orderId, Model model, Principal principal,
            @RequestParam(required = false) Long userId) {
        Optional<User> userOpt = (userId != null)
                ? userService.findById(userId)
                : (principal != null ? userService.findByEmail(principal.getName()) : Optional.empty());
    ...
    @PostMapping("/orders/product-service/{orderId}/submit-issue")
    public String submitIssue(@PathVariable Long orderId, Principal principal,
            @RequestParam(required = false) Long userId,
        ...
        complaintService.submitComplaint(user, order, product, issueType, description);

        redirectAttributes.addAttribute("userId", user.getId());
        return "redirect:/profile/orders?success=Complaint submitted successfully";
    }
